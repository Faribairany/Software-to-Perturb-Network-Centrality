...
#In this method after adding nodes found using the snowball expansion, neighbours of the nodes foi=und by expansion is added until n percent node is found 
...
def write_headers_to_file_for_each_core(outfile_conn_comp):
        ...
        file1.write(",")
        file1.write("List_of_high_centrality_nodes_in_the_core")
        file1.write(",")
...
def write_to_file_for_each_core(outfile_conn_comp,core_num,num_of_high_centrality_node_list_for_the_core, high_centrality_node_list_for_the_core, num_of_not_high_centrality_node_list_for_the_core):
    ...
    file1.write(str(high_centrality_node_list_for_the_core))
    file1.write(",")
...
def find_subgraph_around_seed_node(node_already_added,list_of_neighbours_of_node_already_added,seed_node,nodelist):  
    #print(seed_node)      
    ...
    #list sub is the list of neighbours of the already added node to the list
    list_sub=list_of_neighbours_of_node_already_added+node_already_added
    #list_sub.append(seed_node)
    #list to be subtracted
    for element in neighbour_list_for_seed:
           ...
           # print(neighbour_list_for_element)
            ...
            new_list=list(set(neighbour_list_for_element) - set(list_sub))
...
def neighbours_of_next_node_already_added(next_node):
     #find the list of neighbours from csv
    neighbour_list=find_neigbours_list(next_node)
    #print(neighbour_list_for_seed)

    #if neighbour_list_for_seed!="NULL":
        #remove the punctuation from the list
    neighbour_list=remove_punc(neighbour_list)
        #count the list of elements in the list
    neighbour_list=list_of_negibours(neighbour_list)
        # to remove duplicated 
...
#implementation of snowboll algorithm
def snowball_algo(n_percent_value, V, filename,adjacency_list,nodelist,listname,seed_node_list,outfile_modified_edge_list,outfile,outfile_neighbor_no,outfile_node):   
    x = filename.split(".")
    #calculate the number of adjacency nodes to be removed from the list
    N_percent_no_num=node_number(V,n_percent_value)    
    i=0
    for seed_node in seed_node_list:
            ...
            list_of_neighbours_of_node_already_added=[]
            list_of_neighbours_of_node_already_added=list_of_neighbours_of_node_already_added+ neighbours_of_next_node_already_added(seed_node)        
            ...
            #print(len(node_already_added))      
                    next_node=find_subgraph_around_seed_node(node_already_added,list_of_neighbours_of_node_already_added,seed_node,nodelist)
                    if next_node!="NULL":
                        list_of_neighbours_of_node_already_added=list_of_neighbours_of_node_already_added+neighbours_of_next_node_already_added(next_node)
                        # from list 
                    res = []
                    [res.append(x) for x in list_of_neighbours_of_node_already_added if x not in res]
                    list_of_neighbours_of_node_already_added=res
                    seed_node=next_node
            ...
            percentage_of_node_already_added_to_be_taken=((10*n_percent_value)/100)*len(node_already_added)
            #print(len(node_already_added))
            node_already_added = list(itertools.islice(node_already_added, int(percentage_of_node_already_added_to_be_taken)))  
            len_of_neighbours_to_be_added_to_new_list=int(N_percent_no_num)-len(node_already_added)
            
            #nodelist containing n percent node found by adding some of the neighbours of node already added list to make it n percent
            new_list=[]
            for val in list_of_neighbours_of_node_already_added:
                exist=val in node_already_added
                if exist==False:
                    new_list.append(val)
                    if len(new_list)==len_of_neighbours_to_be_added_to_new_list:
                        break
            
            node_already_added=node_already_added+new_list
            ...
            outfile_final=x[0]+"_core_no_and_node_info_" + str(n_percent_value) + "_percent_node_" + listname + number +  ".csv"
            #outfile_final=x[0]+"_core_no_and_node_info_high_deg_and_high_clus(snowball).csv"
            # outfile_final=x[0]+"_core_no_and_node_info_high_deg(snowball).csv"
            # outfile_final=x[0]+"_core_no_and_node_info_high_clus(snowball).csv"
            ...
            for keys in flipped:
                        ...
                        core_num=keys
                        write_to_file_for_each_core(outfile_final,core_num,num_of_high_centrality_node_list_for_the_core,high_centrality_node_list_for_the_core, num_of_not_high_centrality_node_list_for_the_core)
...
if __name__ == "__main__":
    ...
    #high clus coefficient
    random_node_as_seed=['12','1101','2040','2376','6438']
    high_deg_and_clus_cof_as_seed=['368', '224', '369', '309', '195']

    #use random seeds for 
    snowball_algo(10, V, filename,adjacency_list,nodelist,"random", random_node_as_seed,outfile_modified_edge_list,outfile,outfile_neighbor_no,outfile_node)
    snowball_algo(10, V, filename,adjacency_list,nodelist,"high_deg_and_high_clus", high_deg_and_clus_cof_as_seed,outfile_modified_edge_list,outfile,outfile_neighbor_no,outfile_node)
    snowball_algo(5,V, filename,adjacency_list,nodelist,"random", random_node_as_seed,outfile_modified_edge_list,outfile,outfile_neighbor_no,outfile_node)
    snowball_algo(5,V,filename,adjacency_list,nodelist,"high_deg_and_high_clus", high_deg_and_clus_cof_as_seed,outfile_modified_edge_list,outfile,outfile_neighbor_no,outfile_node)
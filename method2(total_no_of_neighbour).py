...
def find_subgraph_around_seed_node(node_already_added, seed_node):  
    ...
    # from list 
        ...
        #check if the node itself present in the adjacency list
        exist=seed_node in neighbour_list_for_seed
        #remove the node for which the neighbours are to be found to avoid the self loops
        if exist==True:
            neighbour_list_for_seed.remove(seed_node)
        dict_node_with_neighbour_num={}
        for element in neighbour_list_for_seed:
            #find the list of neighbours from csv
            neighbour_list_for_element=find_neigbours(element)
            #remove the punctuation from the list
        ...
        #  neighbour_list_for_element=len(neighbour_list_for_element)
            #count_no_of_negibours split the string with space put elements in a list and count the number of elements
            dict_node_with_neighbour_num[element]=int(neighbour_list_for_element)
    ...
    # Remove multiple keys from dictionary
        ...
        #to choose only the nodes not in the nodelist
        #exist=next_node_to_be_added in node_already_added
        #while(exist==True):
            #Deleting an item from dictionary using del
        #   del dict_node_with_neighbour_num[next_node_to_be_added]
        #   next_node_to_be_added=max(dict_node_with_neighbour_num, key=dict_node_with_neighbour_num.get)
        #  exist=next_node_to_be_added in node_already_added
    else:
        next_node_to_be_added="NULL"
...
if __name__ == "__main__":
   ...
   # write_neighbour_info_to_file(outfile,outfile_node,outfile_neighbor_no,adjacency_list)
    ...
    #Method1(choose a node with high degree and increment the size of the graph choose a neighbour with highest no of neighbours), in this way continue until 10% nodes are includes in the subgraph 
    #['164','612','577','1355','158']
    seed_node='158'
    print(seed_node)
    ...
    while(len(node_already_added)!=int(N_percent_no_num)):
        ...
        if len(node_already_added)==46:
            print('ok')
        next_node=find_subgraph_around_seed_node(node_already_added,seed_node)
        seed_node=next_node
    ...
    high_centrality_node_list_for_the_conn_comp=intersection(high_cent_node, not_not_added_to_list)
    #list of not high centrality node in connected component
    not_high_centrality_node_list_for_the_conn_comp=[i for i in not_not_added_to_list if i not in high_cent_node]
    ...
    outfile_final=x[0]+"_core_no_and_node_info_high_degree.csv"
